"""Email content generation for Iridia Daily research intelligence."""
...
class EmailGenerator:
    def generate_html_email(self, papers, summaries, date_str):
        """Generate HTML email with multiple research papers."""
        paper_sections = "".join([
            self._create_paper_section_html(paper, summary, i == 0, i == len(papers) - 1)
            for i, (paper, summary) in enumerate(zip(papers, summaries))
        ])
        
        return self._get_html_template(paper_sections, date_str, len(papers))
    
    def generate_plain_text_email(self, papers, summaries, date_str):
        """Generate plain text version of the email."""
        lines = [f"IRIDIA DAILY RESEARCH INTELLIGENCE\n{date_str}", "=" * 50]  
    ...
    def _create_paper_section_html(self, paper, summary, is_first, is_last):
        """Create HTML section for a single paper."""
        accent_color, category = get_category_info(summary)
        
        divider_html = "" if is_last else f"""
    ...
    def _get_html_template(self, paper_sections, date_str, num_papers):
        ...
        <tr><td align="center">
            <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: var(--card-bg); border-radius: 16px; overflow: hidden; box-shadow: var(--shadow);">
                ...
                <tr><td style="padding: 32px 40px; background-color: var(--card-bg); border-top: 1px solid var(--border-color);">
                    ...
                    <p style="margin: 0; color: var(--text-secondary); font-size: 12px;">© 2025 Iridia Daily. Intelligence worth sharing.</p>
                </td></tr>
